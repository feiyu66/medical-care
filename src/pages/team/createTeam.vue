<template>
  <div class="box">
      ...
      <div class="teamName">
        <text class="team_name">团队名称：</text>
        <input @input="inputWord" class="team_ipt" type="text" placeholder="请输入团队名称">
      </div>
...
export default {
  ...
  created () {
    ...
    if (WXEnvironment.platform == "Web") {
      this.scrollStyle = {
        width: "750px",
        height: Utils.env.getPageHeight() - 90 - 140 + "px"
      };
    } else {
      this.scrollStyle = {
        width: "750px",
  ...
  methods: {
    teamGetBindableNurses: async function () {
      ...
      api.get(`${this.baseURL}health/doctor/nurses?`, params, data => {
        console.log(data)
        if (data.code == 200) {
    ...
    minibarRightButtonClick () {
      if (this.teamName == '') {
        modal.toast({ message: '团队名称未填写！', duration: 1 });
      } else {
        console.log(this.nurseIdData.join(','))
        let params = {
          userId: this.drId,
          nurseIds: this.nurseIdData.join(','),
          teamName: this.teamName
        }
        console.log(params)
        api.jsonpost(`${this.baseURL}health/team/create?`, 
